...
void loop() {
  ...
  Serial.println(sensorR3);
  Serial.println(sensorR2); 
  Serial.println(sensorR1);
  Serial.println(sensorL1);
  Serial.println(sensorL2);
  Serial.println(sensorL3);
  double value; 
  if((sensorR3 + sensorR2 + sensorR1 + sensorL1 + sensorL2 + sensorL3) == 0)
  {
    value = 0;
  }
  else
  {
    value = ( 2 * sensorR2 + 1 * sensorR1 - 1 * sensorL1 - 2 * sensorL2 ) / ( sensorR2 + sensorR1 + sensorL1 + sensorL2 );
  }

  double Left = 160 - 30 * value;
  double Right = 160 + 30 * value;
  ...
  for(int i = 0;i < 25;i++)
  {
    MotorWriting(250 - 10 * i,250 - 10 * i);
    delay(1000);
  }
  */
